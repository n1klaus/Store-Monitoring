...
class StoreHandler(BaseModel):
    ...
    def calculate_uptime(self) -> dict:
        ...
        for store_id, values in self.results.items():
            ...
            last_timestamp_local: datetime = last_timestamp_utc.astimezone(store_timezone)
            ...
            store: StoreView = session.query(Store).filter(Store.store_id == store_id).first()
            if not store:
            ...
            if not store_status == 'active':
                ...
                if CURRENT_DATETIME.hour == store.last_known_uptime_utc.hour:
                    uptime_hour = timedelta(CURRENT_DATETIME.min - store.last_known_uptime_utc.min)
                else:
                ...
                if CURRENT_DATETIME.day == store.last_known_uptime_utc.day:
                    uptime_day = timedelta(CURRENT_DATETIME.hour - store.last_known_uptime_utc.hour)
                else:
                ...
                if CURRENT_DATETIME.day - 7 <= store.last_known_uptime_utc.day:
                    diff = CURRENT_DATETIME.day - store.last_known_uptime_utc.day
                    uptime_week = timedelta(CURRENT_DATETIME.hour - store.last_known_uptime_utc.hour)
                else:
                    top_up_1 = abs(24 - timedelta(CURRENT_DATETIME.hour))
                    top_up_2 = abs(24 - timedelta(store.last_known_uptime_utc.hour))
